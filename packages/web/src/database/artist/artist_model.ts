...
type params =
	| [string, "Person", shapeOfGroup?]
	| [string, "Group", shapeOfPerson?]

export async function findArtistByKeyword(
	...[keyword, type, shape = undefined]: params
) {
	if (type === "Person") {
		return await e
			.select(e.Artist.Group, (a) => ({
				...(shape ?? e.Artist.Group["*"]),
...
await findArtistByKeyword("foo", "Person")
