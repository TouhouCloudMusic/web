...
function getVariantStyle(
	...
	return styleRecord[
		props.color == "warning" ? "reimu" : (props.color ?? "gray")
	]
}

export function PrimaryButton(props: ButtonProps) {
	// @tw
	const baseStyle = "shadow-2 text-white"
	const styleRecord: Record<ValidColor, string> = {
		gray:
			// @tw
			`
			dark:text-gray-1200
		...
		blue:
			// @tw
			`
			bg-blue-600 hover:bg-blue-500 active:bg-blue-400 disabled:bg-blue-400
			dark:disabled:bg-blue-600
			`,
		reimu:
			// @tw
			`
			bg-reimu-700 hover:bg-reimu-600 active:bg-reimu-500 disabled:bg-reimu-400
	...
	const finalProps = propPreprocessor(props, () =>
		baseStyle.concat(getVariantStyle(props, styleRecord))
	)
