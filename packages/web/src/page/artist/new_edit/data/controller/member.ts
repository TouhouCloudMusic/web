import { type FormStore, getValues, insert, remove } from "@modular-forms/solid"
...
export class MemberController {
	...
	add(newArtist: ArtistByKeyword) {
		...
		if (newArtist.artist_type === this.store.artistType) return
	...
	async serach(keyword: string) {
		...
		const artistType = this.store.artistType === "Person" ? "Group" : "Person"
