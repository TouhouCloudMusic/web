...
import * as Arr from "~/lib/validate/array"
...
export class ArtistFormHelper {
	...
	private get initMemberIDList(): NonEmptyArray<string> | undefined {
		...
		if (this.initData.artist_type == "Group") {
			...
			if (Arr.isNotEmptyArray(res)) return res
		} else {
	...
	private get insertBaseShape() {
		return {
			...
			...(Arr.isNotEmptyOrNone(this.strMemberList) ?
	...
	public get hasLinkMemberList() {
		return Arr.isNotEmptyOrNone(this.linkMemberList)
	}

	private get linkMemberJsonDataExp() {
		if (Arr.isEmptyOrNone(this.linkMemberList)) {
	...
	private get linkMembersQuery() {
		if (Arr.isEmptyOrNone(this.linkMemberList)) return undefined
		if (this.isPerson) {
			return null
		} else {
			const query = e.op(
	...
	public get hasUnlinkMemberOfList() {
		return Arr.isNotEmptyOrNone(this.unlinkMemberOfList)
	}
	...
	public unlinkMemberOfQuery = (uuid: uuid) => {
		...
		if (!Arr.isNotEmptyOrNone(unlinks)) {
			throw new Error(
