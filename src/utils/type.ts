...
type LastOf<T> =
  UnionToIntersection<T extends unknown ? () => T : never> extends () => infer R ? R
  : never
...
export type LessThan<A extends number, B extends number, T extends unknown[] = []> =
  T["length"] extends A ?
  T["length"] extends B ?
  false
  : true
...
export type SafeOmit<T extends Record<PropertyKey, unknown>, K extends keyof T> = {
